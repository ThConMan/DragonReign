...
# The staged libs/paper-server-26.2.jar is a Paperclip launcher — it does not
...
if [ ! -d libraries ]; then
  ...
  java -Dpaperclip.patchonly=true -jar libs/paper-server-26.2.jar
fi
...
javac --release 25 -cp "$CP" -d build/classes @build/sources.txt
